...
parser.add_argument("--query", type=str, default=None, help="Query string for RAG or search mode")
...
if __name__ == "__main__":
    ...
    if args.mode == "rag":
        ...
        if not args.query:
            ...
            print(
                "Usage: python main.py --mode search --query 'your query' --search-method [boolean|semantic|bm25|faiss|fused]"
            )
            exit(1)
        ...
        if args.search_method == "faiss":
            ...
            faiss_mapping = load_faiss_mapping(config.FAISS_MAPPING_PATH)
            faiss_model = SentenceTransformer(config.MODEL_NAME)
            search_fn = lambda q: faiss_query(
                q, faiss_index, faiss_mapping, faiss_model, top_k=config.DEFAULT_TOP_K
            )
